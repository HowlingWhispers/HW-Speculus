...
import { z } from 'zod';
import { publicV3Package, v3LaunchSchema, type V3LaunchPackage } from '../../src/v3/contracts/launch.js';
...
function packagedWorldId(value: V3LaunchPackage): string | null {
...
function seal(value: V3Authorization): string {
...
function authorization(request: Request, id: string): V3Authorization | null {
...
export function createV3Router(options: { production?: boolean } = {}) {
  const router = Router();
  const deposits = new Map<string, V3Deposit>();
  ...
  router.post('/launch', (request, response, next) => {
    try {
      ...
      response.status(201).json({ launchUrl: `${origin}/?launch=${encodeURIComponent(code)}`, expiresAt: parsed.package.expiresAt });
  ...
  router.post('/research/retract', async (request, response, next) => {
    try {
      ...
      if (options.production && origin !== expectedOrigin) return response.status(403).json({ error: 'Invalid V3 request origin.' });

      const body = researchRetractionSchema.parse(request.body);
      const session = authorization(request, body.launchId);
      if (!session) return response.status(401).json({ error: 'V3 authorization expired. Launch this record again from Orbis.' });
  ...
  router.post('/generate', async (request, response, next) => {
    try {
      ...
      if (options.production && origin !== expected) return response.status(403).json({ error: 'Invalid V3 request origin.' });
      const body = generationSchema.parse(request.body);
      const session = authorization(request, body.launchId);
      if (!session) return response.status(401).json({ error: 'V3 authorization expired. Launch this record again from Orbis.' });
      if (body.model !== session.model) return response.status(403).json({ error: 'Model does not match the authorized V3 launch.' });
